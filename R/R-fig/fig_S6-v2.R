...
phi_result_tibble_chessfide <- f_calculus_F_data(vector_entry = seq(1, length(vector_of_ids_rank[[k]]), 100), 
                                                 times = length(vector_of_ids_rank), 
                                                 list_entry = vector_of_ids_rank,
                                                 N = 325190, vec_not_continuous = TRUE) %>%
  mutate(N = 325190) %>%
...
### Ploting result ###
phi_data_plot <- phi_result_tibble_universities %>%
...
phi_data_plot %>%
  ...
  scale_color_manual(values = c('#000000', '#009e73', '#f0e442')) +
  theme_bw() +
  xlab(TeX(r"($N_{0}/N$)")) +
  ylab(TeX(r"($\phi$)")) +
  f_data_plot %>%
...
ggsave(filename = "figures/phi_and_F_opensystems.pdf", width = 18, height = 10, dpi = 300, units = 'cm')
